...
void Table::addEntry( const QString &desc, double theLow){
    ...
    //qDebug()<<model_->item(model_->rowCount()-1,0)->text().toUtf8()<<model_->item(model_->rowCount()-1,1)->text()<<model_->item(model_->rowCount()-1,2)->text()<<model_->item(model_->rowCount()-1,3)->text();
    in = 0;
}

void Table::deleteEntry(){
        ...
        while (!xml.atEnd()){
            if(xml.isStartElement()){
                ...
                if(xml.name() == "element"){
                    ...
                    double vel =  xml.attributes().value("value").toUtf8().toDouble();
                    addEntry(xml.attributes().value("name").toString(), vel);
                }
                xml.readNext();
            }
            xml.readNext();
